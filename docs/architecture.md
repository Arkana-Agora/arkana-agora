...
- **Auth business logic is fully implemented (Módulo 1 — Sprint 1).** Everything else in this document describes the **documented design** (SDD, ADRs, `.specs/`) — i.e. the **planned** architecture. The Auth.js v5 login layer (magic link + Google OAuth, ADR-010) and the design system (Tailwind CSS 4 + shadcn/ui radix-nova preset, oklch light/dark tokens, `next-themes`) shipped in Sprint 0 (F2A/F2B). **Módulo 1 Auth (Sprint 1) shipped**: `POST /api/v1/auth/register` (T6), `POST /api/v1/auth/login` (T7), `POST /api/v1/auth/magic-link` (T9), `POST /api/v1/auth/magic-link/verify` (T10), `POST /api/v1/auth/forgot-password` (T11), `POST /api/v1/auth/reset-password` (T12), `POST /api/v1/auth/refresh` (T13), `POST /api/v1/auth/logout` (T14), `POST /api/v1/auth/verify-email` (T30) and `POST /api/v1/auth/verify-email/resend` (T30) are implemented, backed by `src/services/token-service.ts` (RS256 access 15min + refresh session 30d rotation + `revokeRefreshSession`/`revokeAllSessions`), `src/lib/rate-limit.ts` (in-memory: account lockout 5 failures → 15min, IP volume 5/15min → 429, magic link 3/h per email + 3/h per IP, register 3/15min per email + 3/h per IP, forgot-password 3/h per email + 5/h per IP, verify-email/resend 1/min per email + 5/h per IP, restore-account 3/h per email; reset-password has **no dedicated limiter**), `src/lib/redis.ts` (singleton, gated on `REDIS_URL`), and `src/lib/validators/auth.ts` (`loginSchema`/`magicLinkSchema`/`magicLinkVerifySchema`/`forgotPasswordSchema`/`resetPasswordSchema`/`verifyEmailSchema`/`verifyEmailResendSchema`). Frontend auth shipped: `AuthGuard` (`src/components/auth/auth-guard.tsx`, T24 — client-side route guard with `requiredRole?: UserRole` (`"USER" | "PROFESSIONAL" | "ADMIN"`), starts unverified with `role="status"` skeleton, post-mount `refreshSession()` validation, `/login` redirect) and the Zustand auth store (`src/stores/auth-store.ts`, T24/T25 — `refreshSession()` `POST /api/v1/auth/refresh` cookie-based, returns boolean, preserves `user` on success and on transient `network_error`/`server_error`, clears on `auth_failed`/`bad_response`; T25 completed the store: `persist` middleware (localStorage key `arkana-auth`), `User.emailVerified` + `PartialUser` (C9), `isAuthenticated` derived from `user != null && user.emailVerified`, 5s error auto-clear via store timer, `loginWithGoogle`/`logout`/`deleteAccount`). LGPD account deletion is shipped: `DELETE /api/v1/auth/account` (T15, atomic soft delete via `softDeleteAccount`) + `GET /api/cron/hard-delete` (T16, Vercel Cron 03:00 UTC, `src/jobs/hard-delete-accounts.ts` — anonymization past the 30-day window). Payments, AI, SSE, social, and admin are not implemented.
...
| Pino + Sentry + PostHog + Vercel Analytics | Logging, error tracking, analytics, Web Vitals | MVP — Pino/Sentry implemented in F4 (Sentry disabled without DSN); PostHog events implemented T115 (consent-gated, session recording + dead-click capture off, init skipped in development); Vercel Analytics still planned | `docs/02-architecture/observability.md` |
...
Auth.js v5 (`next-auth@5.0.0-beta.32`, ADR-010) as the MVP login layer: magic link 15 min single-use (`EmailProvider` → `VerificationToken`) + Google OAuth (binding via `User.provider`/`providerId`, no `Account` model), JWT strategy — the session is the Auth.js JWT cookie via `/api/auth/*` (`src/app/api/auth/[...nextauth]`). Route protection is three-layered: `src/proxy.ts` (Next 16, matcher `/dashboard/:path*`, `/perfil/:path*`, `/tirar`, `/tiragem/:path*`, `/minhas-tiragens`, `/meu-arcano/:path*`; unauthenticated → `/login?callbackUrl=<pathname>`) + the route-group guard at `src/app/(app)/layout.tsx` (`auth()` → `redirect("/login")`, F2B defense-in-depth) + the client-side `AuthGuard` (`src/components/auth/auth-guard.tsx`, T24 — verifies via `refreshSession()` on the auth store, optional `requiredRole`, skeleton loading, `/login` redirect on failure). **Sprint 1 (Módulo 1 Auth) — implemented**: `POST /api/v1/auth/register` (T6), `POST /api/v1/auth/login` (T7), `POST /api/v1/auth/magic-link` (T9), `POST /api/v1/auth/magic-link/verify` (T10), `POST /api/v1/auth/forgot-password` (T11), `POST /api/v1/auth/reset-password` (T12), `POST /api/v1/auth/refresh` (T13), `POST /api/v1/auth/logout` (T14), `POST /api/v1/auth/verify-email` (T30) and `POST /api/v1/auth/verify-email/resend` (T30) with the Custom JWT Layer (`src/services/token-service.ts`: `signAccessToken` RS256 15min claims role/plan/tokenVersion, `verifyAccessToken` fail-closed Redis+DB, `createRefreshSession` 30d, `rotateRefresh` rotation + family revoke, `bumpTokenVersion`, `revokeRefreshSession`, `revokeAllSessions`; `src/lib/rate-limit.ts`: account lockout 5 failures → 15min `AUTH_ACCOUNT_LOCKED` retryAfter 900, IP volume 5/15min → 429 `AUTH_RATE_LIMITED`, magic link 3/h per email → 429 `AUTH_MAGIC_LINK_RATE_LIMIT`, register 3/15min per email + 3/h per IP → 429 `AUTH_RATE_LIMITED`, forgot-password 3/h per email → 429 `AUTH_FORGOT_RATE_LIMIT` + 5/h per IP (same code), verify-email/resend 1/min per email + 5/h per IP → 429 `AUTH_RATE_LIMITED`; reset-password has no dedicated limiter (single-use 1h token); `src/lib/redis.ts` singleton gated on `REDIS_URL`). Remaining Sprint 1: Facebook OAuth. LGPD delete (`DELETE /api/v1/auth/account`) **implemented** (T15). LGPD hard-delete cron (`GET /api/cron/hard-delete`, Vercel Cron 03:00 UTC, `src/jobs/hard-delete-accounts.ts`) **implemented** (T16). RBAC roles `USER → PROFESSIONAL → ADMIN` (implemented in `prisma/schema.prisma`) plus `SUPER_ADMIN` **[planned]**; plan tier (`UserPlan`: FREE/PLUS) is a separate dimension from role. Permission matrix enforced by middleware (`docs/07-security/permissions.md`).
