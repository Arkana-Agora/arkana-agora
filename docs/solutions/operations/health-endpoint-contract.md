...
- **2026-08-24:** Redis joined the envelope as an **optional** service — `services` is now `{ database, redis }`, probes run in parallel via `Promise.allSettled`, and `checkRedis()` gates on `REDIS_URL`: absent → `{ status: "not-configured" }` (neutral); configured-but-failing (TCP PING time-boxed 3s via `node:net`) → `error` (degrades to 503). Failure logging migrated from `console.error` to Pino (`@/lib/logger`, `[health]` prefix preserved). Tests rewritten deterministically with a mocked Prisma singleton (`vi.mock("@/lib/prisma")`) pinning all three derivation branches: ok+not-configured→200, db error→503, redis error+db ok→503. Sentry integration landed in the same phase but does not alter this contract.
- **2026-09-10:** Refresh of pattern registry. Current status: health endpoint fully resolved with derived status, time-boxed DB check, Pino logging, APP_VERSION from `src/lib/version.ts`. Redis is optional service with neutral `not-configured` semantics. Tests pin the contract independently of DB state. Pattern is current and actively maintained.
