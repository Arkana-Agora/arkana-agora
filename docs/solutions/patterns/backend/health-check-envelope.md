...
- Every check **must** catch and log via Pino (`logger.error({ err }, "[health] ...")` from `@/lib/logger`) and return a status object — never let it throw out of `GET`.
...
Adding a real service check while keeping the contract (Redis followed this exact path when it joined the envelope — see the snapshot above; the future `checkAI()` will too):
...
- [ ] Top-level `status` is **derived** from the checks (`ok`/`degraded`) and must never contradict the HTTP code (pinned by `tests/health.test.ts`). With `{ database, redis }` in the envelope today, `degraded` ⇔ DB check `error` **or** configured-but-failing redis.
- [ ] `not-configured` is **neutral** — it never contributes to the failure aggregate and never triggers 503. Applies to optional services in the envelope (redis today; AI when it joins — contract per `observability.md` §6.3).
...
- [ ] Failure logging uses Pino via `@/lib/logger` (`logger.error({ err }, "[health] ...")`) — never bare `console.*`.
...
1. **Adding a service check:** extend the status union type (e.g. `AIStatus`), write `checkX()` with a time-boxed probe + catch + `logger.error({ err }, "[health] ...")`, add `x: await checkX()` to the `services` map (redis joined exactly this way).
