...
2. **E-mail + senha**: validação client-side (react-hook-form + `zodResolver(loginSchema)`); a página `/login` define o cookie CSRF (`csrf-token`/`__Host-csrf-token`) e `useAuthStore.login(email, password)` chama `POST /api/v1/auth/login` enviando o header `x-csrf-token` (double-submit — 403 `CSRF_TOKEN_INVALID` sem ele); sucesso → `/dashboard`; `AUTH_EMAIL_NOT_VERIFIED` → `/verify-email?email=...`; demais erros mapeados para mensagens amigáveis
...
- **CSRF** — double-submit obrigatório (cookie `csrf-token`/`__Host-csrf-token` definido pela página `/register` vs header `x-csrf-token`); 403 `CSRF_TOKEN_INVALID` em falha
...
- **CSRF (2026-09-18)** — double-submit obrigatório: cookie `csrf-token`/`__Host-csrf-token` (definido pela página `/login`) vs header `x-csrf-token` (enviado pelo `login()` do store); validado logo após o body, antes de qualquer efeito colateral
...
`AUTH_SECRET`). `tests/auth-store.test.ts` — 79 testes vitest do AuthStore (login: sucesso,
...
| Token interceptor (`src/lib/api.ts`) | Sprint 1 — **T26 implementado** — instância Axios com adapter custom (fetch), request interceptor adiciona `Authorization: Bearer <accessToken>` via `getSession()` (Auth.js session → `session.accessToken`), response interceptor trata `401` → chama `POST /api/v1/auth/refresh` (cookie-based), aguarda novo token via `getSession()`, re-tenta request original; previne loop com flag `_retry` no request + `refreshPromise` singleton (concurrent 401s aguardam mesmo refresh); `baseURL: "/api/v1"`, `adapter: fetchAdapter` (resolve URLs relativas via `window.location.origin` + `credentials: "include"`) |
