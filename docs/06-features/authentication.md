...
- **Cadastro via OAuth social** (Google, Apple, Facebook) com mapeamento automático de dados
- **Login por magic link** enviado ao e-mail cadastrado, válido por 10 minutos
...
6. O sistema gera um access token (15 min) e um refresh token (7 dias)
7. O refresh token é armazenado em cookie httpOnly; o access token é retornado no body da resposta
...
11. O usuário pode solicitar exclusão da conta, com confirmação via e-mail e grace period de 7 dias
...
| Google OAuth / Apple Sign-In | API externa | Autenticação social |
...
- **CA-04**: A exclusão de conta deve marcar o registro para exclusão em 30 dias, com possibilidade de reativação em até 7 dias
