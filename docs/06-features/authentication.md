...
`AUTH_SECRET`). `tests/auth-store.test.ts` — 33 testes vitest do AuthStore (login: sucesso,
...
`UNEXPECTED_RESPONSE`, token vazio → `AUTH_RESET_TOKEN_INVALID` sem fetch; `clearError`).
