...
- **AuthStore (Zustand)** — Sprint 1: **protótipo mínimo (T19/T20/T21)** — `src/stores/auth-store.ts` com `user`, `isAuthenticated`, `isLoading`, `error`, `login(email, password)` (POST `/api/v1/auth/login`), `register(data: RegisterInput)` (POST `/api/v1/auth/register`), `sendMagicLink(email: string)` (POST `/api/v1/auth/magic-link`) e `clearError`; `login()`, `register()` e `sendMagicLink()` endurecidos (review 6-agentes): try/catch/finally com `isLoading` sempre resetado, respostas tipadas via uniões `LoginResponse` / `RegisterResponse` (`{ user, message } | { error: { code, message } }`) e `sendMagicLink` retornando `MagicLinkResult` (união discriminada `{ success: true, message? } | { success: false, code, message?, retryAfter? }`), `User` alinhado ao payload da API (`{ id, name, email, displayName, role, plan, avatar }`, sem `emailVerified`) e `export type { User }`; **register não faz auto-login** — `isAuthenticated` permanece `false`, `user` permanece `null`; `sendMagicLink` não autentica — apenas envia o link, `isAuthenticated` permanece `false`, `user` permanece `null`; persistência (T25) e interceptor Axios (T26) pendentes
...
`AUTH_SECRET`). `tests/auth-store.test.ts` — 17 testes vitest do AuthStore (login: sucesso,
...
rede → `NETWORK_ERROR`, resposta não-JSON; `clearError`). `tests/magic-link-form.test.tsx` —
17 testes vitest do MagicLinkForm (renderização, validação client-side, submissão com
loading/sucesso/erros mapeados, cooldown de reenvio de 60s com fake timers).
...
| AuthStore (Zustand, `src/stores/auth-store.ts`) | Sprint 1 — **protótipo mínimo (T19/T20/T21); `login()`/`register()`/`sendMagicLink()` endurecidos (try/catch/finally, `LoginResponse`/`RegisterResponse`/`MagicLinkResult` tipadas, `User` sem `emailVerified`); `register` não faz auto-login; `sendMagicLink` não autentica; persistência pendente (T25)** |
