...
- **Auth**: login layer (MVP) at `/api/auth/*` (Auth.js v5 — ADR-010): Google OAuth + magic link (`EmailProvider`, 15 min single-use). Sprint 1: `POST /api/v1/auth/register|login|magic-link|magic-link/verify|refresh|logout|forgot-password|reset-password`, `GET /api/v1/auth/me`, Custom JWT Layer (access 15 min RS256, refresh 30 days opaque with rotation), Facebook OAuth. **Implemented (Módulo 1 Auth):** `POST /api/v1/auth/register` (T6), `POST /api/v1/auth/login` (T7), `POST /api/v1/auth/magic-link` (T9), `POST /api/v1/auth/refresh` (T13) and `POST /api/v1/auth/logout` (T14) — `src/services/token-service.ts`, `src/lib/rate-limit.ts` (+ magic link 3/h per email), `src/lib/redis.ts`, `src/lib/validators/auth.ts` (`loginSchema`/`magicLinkSchema`). See `docs/04-api/authentication.md`.
