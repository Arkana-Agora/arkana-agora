...
> **Integração Sentry (Fase 4 do Sprint 0):** `@sentry/nextjs` inicializado condicionalmente — server/edge via `src/instrumentation.ts` (`NEXT_PUBLIC_SENTRY_DSN`), client via `src/instrumentation-client.ts` (`NEXT_PUBLIC_NEXT_PUBLIC_SENTRY_DSN`). Sem DSN configurado, o SDK permanece desabilitado e o build não depende de credenciais. `next.config.ts` usa `withSentryConfig`; erros globais de render são capturados em `src/app/global-error.tsx`. `sentry.properties`, `.sentryclirc` e `.env.sentry-build-plugin` ficam fora do git e do build context Docker.
