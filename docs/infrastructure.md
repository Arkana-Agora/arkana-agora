...
| Product analytics | PostHog | implementado (T115 — client `src/lib/analytics.ts` consent-gated in init; host default `https://app.posthog.com`; session recording + dead clicks off; Vercel Analytics still planned) |
...
|---|---|---|---|---|
| **Development** | Local development | `http://localhost:3000` | **Prisma Postgres** (pooled `DATABASE_URL` + direct `DIRECT_URL`); Docker Postgres 16 (localhost:5432) offline fallback | `prisma@^7` + `prisma.config.ts`; `prisma postgres link` already run; `bun run dev`; schema sync via `.\node_modules\.bin\prisma migrate dev` (uses `DIRECT_URL`) |
...
| Analytics | PostHog | — | implementado (T115 — consent checked inside `initAnalytics()` + at every `track*`; host default `https://app.posthog.com` via `NEXT_PUBLIC_POSTHOG_HOST`; session recording + dead-click capture disabled; init no-op in development as of 2026-09-24) | Product events (`signup`, `reading`, `ai_interpretation`, `arcana_calculate`); Vercel Analytics for Web Vitals still planned |
...
- **Build command / heap flag (implemented 2026-09-23):** `package.json` `build` = `prisma generate && node --max-old-space-size=4096 node_modules/next/dist/bin/next build` — one entry shared by local, CI (`bun run build` in `ci.yml`), the Docker builder stage, and Vercel. Mitigates intermittent Turbopack/PostCSS `Zone Allocation failed` OOM (exit 134) compiling `globals.css` under low free RAM; the flag is **not** a free-RAM substitute (keep **>1 GB free** before building) and does not propagate to every spawned child. Full analysis: `docs/solutions/ci-cd/turbopack-postcss-oom.md` (indexed as a pattern entry in `docs/runbooks/README.md`).
- **Vercel Cron (implemented, T16):** `vercel.json` defines a daily cron `0 3 * * *` hitting `GET /api/cron/hard-delete` (no `Authorization` field in `vercel.json` — that field is not part of the Vercel cron schema; Vercel automatically sends `Authorization: Bearer <CRON_SECRET>` on scheduled invocations). The route (`src/app/api/cron/hard-delete/route.ts`, `runtime = "nodejs"`) validates the bearer against `CRON_SECRET` (401 on mismatch; 500 `INTERNAL_ERROR` on total job failure) and runs `runHardDeleteJob()` (`src/jobs/hard-delete-accounts.ts`) — LGPD hard-delete/anonymization of accounts past the 30-day soft-delete window. `CRON_SECRET` is a **required production env var** (Vercel env) — without it the cron always returns 401.
- **Migrations**: Prisma Migrate (all environments). Local: Prisma Postgres via `.\node_modules\.bin\prisma migrate dev` (CLI reads `DIRECT_URL` from `prisma.config.ts`); Docker fallback: `docker compose up -d postgres` + same migrate command. The compose `migrate` service (one-shot) and staging/prod CI use `prisma migrate deploy` (`docs/03-database/migrations.md`). **Prisma CLI is pinned to `prisma@^7`** — `prisma@8` RC has no `generate`/`migrate` and breaks `npm run build` (see `docs/solutions/ci-cd/prisma-v8-cli-regression.md`). Migration naming: `YYYYMMDDHHMMSS_descriptive_name`. Invariant: **never edit an applied migration** — create a new one. 6 migrations applied to Prisma Postgres (2026-09-23).
...
- **Observability (implemented F4 + documented targets)**: Pino structured logging implemented (`src/lib/logger.ts` — JSON, redacted fields, pino-pretty in dev); Sentry implemented conditionally (`@sentry/nextjs@10.71.0` via `src/instrumentation.ts` / `src/instrumentation-client.ts` / `global-error.tsx` / `withSentryConfig` — **disabled without DSN**, so builds and runtime never require credentials; production capture requires DSN in the dashboard); PostHog client implemented (T115 `src/lib/analytics.ts`, consent banner; consent also checked **inside** `initAnalytics()` (defense-in-depth, localStorage `analytics-consent`); host default `https://app.posthog.com` (`NEXT_PUBLIC_POSTHOG_HOST`); `disable_session_recording: true` + `capture_dead_clicks: false` pinned locally so remote config cannot re-enable them — 2026-09-23/24; `initAnalytics()` no-ops when `NODE_ENV === "development"` before the key check, so dev never loads PostHog or warns about a missing key; outside development requires `NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN`; covered by `tests/analytics.test.ts`); Vercel Analytics (LCP < 2.5s, INP < 200ms, TTFB < 800ms targets) still planned; `/api/health` implemented with **parallel probes** (`Promise.allSettled`, each time-boxed): envelope `{status,timestamp,version,services:{database,redis}}` — DB hard dependency (5s timeout), redis optional (gate `REDIS_URL`: absent → neutral `not-configured`; configured-but-failing → 503) per `observability.md` §6.3; Prometheus metrics endpoint; Grafana dashboards; PagerDuty+Slack alert rules remain planned. Runbooks exist in `docs/runbooks/` (index: `docs/runbooks/README.md` — Vercel deploy AUTH_URL/GitHub token, local JWT session decryption, + pattern-solution pointers incl. Turbopack/PostCSS OOM); on-call roster and incident post-mortem process still missing.
...
10. **Operational maturity gaps:** `docs/lambdas/README.md` is still an empty template; `docs/runbooks/README.md` now catalogs existing runbooks (Vercel deploy incidents + local build OOM + local JWT session decryption), but there is no on-call roster and no incident post-mortem process yet (security.md defines the incident response *policy* only).
11. **AUTH_URL/AUTH_SECRET production runtime guard (implemented 2026-09-23):** `src/auth/auth.config.ts` fail-fast at module load when `NODE_ENV === "production"` **and** `NEXT_PHASE !== "phase-production-build"` — the **build phase passes**, but **runtime SSR fails** until env is set: requires non-empty `AUTH_URL` with `https://` scheme and non-empty `AUTH_SECRET`; the error carries non-secret diagnostics only (`AUTH_URL_in_env`, `AUTH_URL_empty`, scheme snippet — never secret values). Env edits in the Vercel dashboard require a **redeploy** to take effect (env is not injected into old deployments). Runbook: `docs/runbooks/vercel-deploy-auth-url.md` (§0 diagnostics); Vercel env expectations: `docs/02-architecture/deployment.md` §4.2 (Production `https://arkanaagora.com.br`, Preview `https://arkana-agora.vercel.app`, both + `AUTH_SECRET`); guard tests: `tests/auth-config.test.ts`. Distinct failure from build OOM — see `docs/solutions/ci-cd/turbopack-postcss-oom.md` §Gotchas.
