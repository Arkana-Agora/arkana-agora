...
1. **Nothing is deployed to a cloud provider.** Only the local stack exists: Next.js skeleton (`src/`, `prisma/`, `tests/`) + Docker infra (F1: `docker-compose.yml`, `Dockerfile`, `.dockerignore`). `backend/`/`frontend/` remain empty placeholders; no GitHub Actions workflows, no IaC (Terraform/CDK) exist in the repo. Every provider account, domain, and pipeline described above is **planejado** and must be provisioned before any runtime claim is made.
