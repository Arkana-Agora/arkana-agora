...
- [x] F4: logger Pino (T32)
- [x] F4: health probes paralelas + redis + logger (T33)
- [x] F4: Sentry withSentryConfig + global-error (T34)
- [x] F4: docs observability sync (T35) + verificação (T36)
- [x] F5: doc-shepherd sem contradições (T37)
- [x] F5: plan-sync + flips sprint-0.md (T38)
- [x] F5: M0 checklist validado (T39)
- [x] F5: infrastructure/environments docs sync (T40)
- [x] F5: verificação final full green (T41)
...
- 2026-08-13 — **Step 4 review loop (6 revisores em paralelo: nextjs-reviewer, kieran-typescript-reviewer, code-simplicity-reviewer, architecture-strategist, security-sentinel, workflow/lint)**: 0 CRITICAL; 3 Important corrigidos + 1 descartado; demais itens vira Informational/deferido.
    - **Fix A — guard duplicado**: removido `redirect("/login")` da página `dashboard/page.tsx` (proteção documentada no ADR-010 §7 é de 2 camadas: `src/proxy.ts` + guard `(app)/layout.tsx`; a checagem de página era 3ª camada redundante — `session?.user?.name` com optional chaining preserva a saudação). Arquitetura validada.
    - **Fix B — deps**: `shadcn` (CLI, ~40 transitivos p/ prod) movido de `dependencies` → `devDependencies`; `@hookform/resolvers` **removido** (não consumido — form.tsx usa `Controller` puro RHF; re-adicionar quando resolver Zod chegar, Sprint 1). `bun install` atualizou lockfile (−2 pacotes).
    - **Fix C — teste falso-positivo**: `tests/ui-utils.test.ts` outline agora asserta `border-border` (classe real do light mode; `dark:border-input` isolado em assert próprio) + typo de nome do teste corrigido.
    - **Descartado**: finding "useFormState no useFormField" do kieran — verificado contra `ui.shadcn.com/r/styles/new-york/form.json` (canônico atual): usa o MESMO `useFormContext()` + `getFieldState`; nossa versão é até mais segura (contextos null-checked com erro real vs cast canônico). Sem mudança em `form.tsx`.
    - **Informational (deferidos)**: callbackUrl inconsistente entre proxy/(app) guard/login-form → Sprint 1; SessionProvider fetch extra → otimizar junto do CUSTOM JWT (ADR-009); CSP nonce p/ next-themes; `color-scheme: light` em `:root`; inline theme-provider; hoist ThemeToggle p/ root layout; form.tsx sem uso real ainda (consumido no primeiro form real).
    - Re-verificação pós-fix: lint 0 / type-check 0 / 25 testes verdes / `bun run build` OK (fecha Open Question do nextjs-reviewer sobre RSC boundary + CSS Turbopack).
...
- 2026-08-24 — **Fase 3 executada (T28–T31) + Step 4 review de conformidade**:
    - **T29 ✅**: `prettier.config.mjs` (semi:false, doubleQuote, trailingComma all, width 80; const+export), `.prettierignore`, scripts `format`/`format:check`, Husky pre-commit (`lint-staged` + `tsc --noEmit`), lint-staged em `package.json`. Repo formatado (38 arquivos).
    - **T28 ⚠️→✅**: cadeia `quality → type-check → test → build → deploy` com `postgres:16-alpine` service container e `DATABASE_URL` do serviço no job test (conforme plano). Review de conformidade encontrou **2 defeitos bloqueantes introduzidos pós-implementação**, ambos corrigidos:
        - **[C1]** `bunx prisma db push` no job quality (sem service/DB) falharia sempre → removido;
        - **[C2]** `if:` de job-level referenciava contextos indisponíveis (`secrets.*`, depois `env.*`) → GitHub Actions rejeitaria o workflow inteiro no parse. Substituído por padrão gate-job: `gate-deploy` checa secrets em step (contexto válido) e expõe output `has_creds`; `deploy-staging` condiciona via `needs.gate-deploy.outputs.has_creds == 'true' && push && refs/heads/main`. YAML validado por parser (6 jobs OK).
    - **T30 ⚠️→✅ (código)**: `vercel.json` revertido ao preset mínimo (`framework: nextjs` + security headers `X-Content-Type-Options`/`X-Frame-Options`) — expansão anterior era defeituosa (`outputDirectory .next/standalone` conflita com builder Vercel; `AUTH_URL` fixo de prod vazaria para previews/staging; env `@-refs` legado). Env vars de runtime serão gerenciadas no dashboard Vercel. **Pendente ação manual:** cadastrar secrets `VERCEL_TOKEN`/`VERCEL_ORG_ID`/`VERCEL_PROJECT_ID` no repo GitHub.
    - **T31 ❌ pendente**: exige push remoto (trigger atual: push/PR em `main`) → workflow verde ponta a ponta + staging responde. Master Checklist F3 permanece `[ ]` até T31.
    - **Desvios autorizados fora do escopo F3 (H2, documentados aqui como decisão aceita):** `tsconfig.json` com flags strict adicionais (`noUnusedLocals`, `noUnusedParameters`, `noFallthroughCasesInSwitch`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`) + adaptações decorrentes em `src/auth/prisma-adapter.ts`, `tests/auth.test.ts`; fail-fast de `AUTH_SECRET` em `src/proxy.ts` (throw explícito substitui fallback silencioso `?? ""`); validação de `AUTH_SECRET` em produção em `src/auth/auth.config.ts` (simétrica ao guard de `AUTH_URL`); `next.config.ts` com `images.remotePatterns` para futuro CDN R2 (speculativo, inofensivo). Racional: segurança/tipagem estrita com custo zero validado (lint/type-check/test verdes). Reversão possível se contestado.
    - Verificação local: lint 0 erros · type-check 0 · 25 testes verdes · YAML parse OK. Docs sincronizados: `deployment.md` §6.1/§6.2 (padrão gate + fluxo M0), `infrastructure.md` (status CI/CD implementado).
- 2026-08-24 — **Fix não planejado (pós-push `main`, primeiro deploy Vercel)**: build na Vercel falhava em `onBuildComplete` com `ENOENT .next/next-server.js.nft.json` (commit `a8cfd9e`). Root cause confirmado por pesquisa: regressão upstream Next 16.3 (vercel/next.js#96646, via #93684) — com o adapter da Vercel ativo (`NEXT_ADAPTER_PATH`) o Rust deixa de emitir o NFT do servidor inteiro, mas o finalizador de `output: 'standalone'` lê o arquivo sem guard; local passa porque não há adapter. Fix mínimo em `next.config.ts`: standalone só fora da Vercel (`if (!process.env.VERCEL) nextConfig.output = "standalone"`). Nota de tipagem: a forma inline `process.env.VERCEL ? undefined : "standalone"` **não compila** sob `exactOptionalPropertyTypes` (flag strict adotada no H2) — usar mutação condicional pós-objeto. Anti-padrão descartado (da issue): engolir o ENOENT gera `.next/standalone` quebrado (~48% dos arquivos faltando). Invariante nova: Docker/CI (sem `VERCEL`) preservam `.next/standalone`; reavaliar a condição quando o fix upstream (PR #97287, merged) chegar em release estável do `next`. Docs: `deployment.md` §5.1 nota reescrita + Refresh Note, `infrastructure.md` (2 menções), `monorepo.md`/`sprint-0.md` (anotações de árvore). Verificação local: build `VERCEL=1` OK **sem** `.next/standalone`; build normal OK **com** `.next/standalone`; lint/type-check/format verde; 25 testes. Validação final pendente: push → preview/prod na Vercel conclui `onBuildComplete`.
- 2026-08-24 — **Fix não planejado (2º, CI pull_request #135)**: job `build` falhava no `actions/upload-artifact@v4` com `No files were found with the provided path: .next/` — apesar do `bun run build` ter concluído com sucesso. Root cause: breaking change v4.4.0 do action — arquivos/dirs ocultos (dot-prefixed) são **excluídos por padrão** (`include-hidden-files: false`); `.next/` é dot-dir → glob vazio + `if-no-files-found: error`. Referências: actions/upload-artifact#717, README do action. Fix mínimo: `include-hidden-files: true` no step de upload (`ci.yml`), sincronizado no snippet §6.1 de `deployment.md`. Nota: o artefato `nextjs-build` é informativo — o job deploy-staging usa `amondnet/vercel-action` que clona e builda remotamente, sem consumir o artefato. Verificação local: parse YAML OK com inputs resolvidos (`{name, path, if-no-files-found, include-hidden-files:true}`) · format/lint/type-check verdes. Validação final pendente: re-run do PR #135 verde.
- 2026-08-24 — **Fase 4 executada (T32–T36)**:
    - **T32 ✅**: `src/lib/logger.ts` — Pino (`pino@10.3.1`, `pino-pretty@13.1.3` em dev via `NODE_ENV=development`), `base.service = "arkana-agora"` (formato §2.3), redact de secrets (`authorization`, `cookie`, campos `password`/`token`), serializers std, nível via `LOG_LEVEL` e helper `newReqId()` (`crypto.randomUUID`). `pino` adicionado a `serverExternalPackages` (transports usam worker threads — não deve ser bundled).
    - **T33 ✅**: `src/app/api/health/route.ts` — probes **paralelas** (`Promise.allSettled([checkDatabase(), checkRedis()])`), helper compartilhado `timeBox()` (DB 5s / redis 3s); `checkRedis()` opcional via gate `REDIS_URL`: sem → `{status:"not-configured"}` neutro; com → TCP PING cru (`node:net`, protocolo inline Redis) cuja falha degrada (503). Logging migrado p/ Pino mantendo prefixo `[health]`. Testes reescritos deterministicamente (`vi.mock("@/lib/prisma")`) cobrindo as 3 regras de derivação: ok+not-configured→200 · db error→503 · redis error+db ok→503.
    - **T34 ✅**: `@sentry/nextjs@10.71.0`; init condicional (**sem DSN = desabilitado**, coberto por teste unitário): server/edge `src/instrumentation.ts` (`SENTRY_DSN`, `tracesSampleRate 0.1`, `onRequestError = captureRequestError`), client `src/instrumentation-client.ts` (`NEXT_PUBLIC_SENTRY_DSN`, replay mascarado 1%/10%, ignoreErrors §3.1, `beforeSend` null em dev, export `onRouterTransitionStart` exigido pelo SDK v10); `src/app/global-error.tsx` captura erros de render raiz; `next.config.ts` com `withSentryConfig({silent, widenClientFileUpload})` (nota: `hideSourceMaps` removido no v10 — default; `disableLogger` depreciado e sem suporte Turbopack — removido). `sentry.properties`/`.sentryclirc`/`.env.sentry-build-plugin` fora do git e do build context Docker.
    - **T35 ✅**: docs sincronizados — `observability.md` v1.1 (§6.3 envelope `{database,redis}` + parágrafo Sentry), `health-endpoint-contract.md` + Refresh Note, `health-check-envelope.md` (logger note, snapshot, refresh), `logger-migration-stopgap.md` (Status Update: Phase 2 landed no health route; stopgap restante conhecido: `[auth:magic-link]` em `auth.config.ts:88`, fora do escopo), `environments.md` (env vars Sentry/LOG_LEVEL + linha health check) e `.env.example`.
    - **T36 ✅**: prettier/eslint/type-check verdes; **29 testes** (25→29: +2 health determinísticos, +2 instrumentation); build multi-alvo validado — normal **com** `.next/standalone`, `VERCEL=1` **sem**; builds rodaram sem `NEXT_PUBLIC_SENTRY_DSN` setado (SDK disabled path, zero warnings após ajustes v10). Master Checklist F4 marcada `[x]`.
- 2026-08-24 — **Fase 5 executada (T37–T41) — docs sync + verificação final**:
    - **T37 ✅**: scans de resquícios proibidos (AC#6) limpos — `SQLite dev`/`NEXTAUTH_*`/`GOOGLE_CLIENT_*`: apenas texto histórico legítimo (ADRs, glossários, Refresh Notes); `NextAuth v4` restante só em registros de decisão/sucessão. Passe completo do subagente `doc-shepherd` (60+ docs + 30 `.specs/`): 3 arquivos corrigidos (`deployment.md` §2.3 logger note → Pino implementado + stopgap `auth.config.ts:88`; `admin-health-rich-metadata.md` gotcha → logger landed; `health-check-envelope.md` 9 inconsistências internas → §6.3 resync v1.1, console.error→Pino, constraints/checklist generalizados). Itens sinalizados e preservados como históricos por convenção: narrativa original do `health-endpoint-contract.md` (superseded por Refresh Note), corpo do `logger-migration-stopgap.md`, sample-path §3.1 do `observability.md`, linha dev "Errors: Console" do `environments.md`, ADR-002 (protegido).
    - **T38 ✅**: `sprint-0.md` — flips US-002 (CI entregue F3, E2E pendente de secrets/push) e US-003 (Docker entregue F1 ~32s); task 5 `[x]` F1; tasks 18/19 `[x]` F4 (Sentry condicional / Pino); task 20 envelope atualizado `{status,timestamp,version,services:{database,redis}}` com probes paralelas + redis opcional; critério DB `[x]` (F1 init aplicada); árvore técnica sincronizada (ci.yml, src/auth, lib/logger, instrumentation*, global-error, 29 testes); Entregáveis reorganizados (esqueleto + F1–F4) e Pendentes sem CI/Docker (movidos p/ entregues).
    - **T39 ✅**: `milestones.md` — Status M0 → "Em fechamento (F1–F4 entregues; pendências remotas/produção)"; checklist: docker <60s `[x]` (~32s), PostgreSQL+migrations `[x]` (init), tabelas `[x]` (5 models), monorepo `[x]` (reescopado NA); mantidos `[ ]` os dependentes de validação remota/produção (CI E2E verde, deploy staging, Sentry capturando em prod c/ DSN, docs testada por terceiro) com notas apontando o estado real (implementado/configurado); linha M0 da tabela Resumo atualizada.
    - **T40 ✅**: `infrastructure.md` — header Status estendido (+F3/F4 detalhados); tabelas providers/Core Services: Vercel "planejado"→"em setup (F3, gated por secrets)", Sentry→"@sentry/nextjs… implementado (F4, disabled sem DSN)"; bullet Observability reescrito (Pino/Sentry/health implementados; PostHog/Analytics/Prometheus/Grafana/runbooks planejados). `architecture.md` — Implementation status: envelope `{database,redis}` + probes paralelas, +F3/F4 na lista de entregue, intro da tech stack inclui CI/CD + observability core, row Pino/Sentry com status por componente.
    - **T41 ✅**: verificação final full green — prettier/eslint/tsc 0 erros; **29 testes** vitest verdes; build multi-alvo (normal COM standalone / `VERCEL=1` SEM); Docker Desktop iniciado para a validação → `docker compose up -d --build` OK (postgres healthy, redis up, migrate exited 0, web up) e `/api/health` → **200** `{"status":"ok",...,"services":{"database":{"status":"ok"},"redis":{"status":"ok"}}}` (envelope F4 validado ponta a ponta no container).
    - Master Checklist F5 marcada `[x]`. Pendências remanescentes do plano (fora do controle local): T31/F3 — push remoto + secrets `VERCEL_*` + PR #135 verde + staging respondendo; Sentry em produção exige DSN no dashboard.
