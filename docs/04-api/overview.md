...
> `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`) retornam o
> body **plano (flat)** — `{ user, message }`, `{ accessToken, user }`, `{ accessToken, expiresIn }`
> e `{ message }` respectivamente — **sem** wrapper `data`. Este é o contrato canônico
