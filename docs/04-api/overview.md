...
> `POST /api/v1/auth/forgot-password`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`)
> retornam o
> body **plano (flat)** — `{ user, message }`, `{ accessToken, user }`, `{ message }`,
> `{ accessToken, user }`, `{ message }`, `{ accessToken, expiresIn }` e `{ message }`
> respectivamente — **sem**
